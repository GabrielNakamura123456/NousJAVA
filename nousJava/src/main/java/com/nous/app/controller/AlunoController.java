...
public class AlunoController {
    ...
    public String dashboard(Authentication authentication, Model model) {
        ...
        List<CheckinEmocional> historico =
                checkinService.listarPorUsuario(usuario.getIdUsuario());

        model.addAttribute("usuario", usuario);
        model.addAttribute("historico", historico.stream().limit(5).toList());
        model.addAttribute("temAlerta",
                checkinService.possuiAlerta(usuario.getIdUsuario()));
    ...
    public String salvarCheckin(
            @Valid @ModelAttribute("checkinForm") CheckinEmocionalDTO dto,
        ...
        CheckinEmocional checkinSalvo =
                checkinService.salvar(dto, usuario);

        model.addAttribute("recomendacao",
                checkinService.gerarRecomendacao(checkinSalvo));

        model.addAttribute("checkin", checkinSalvo);
    ...
    public String trilhas(Authentication authentication, Model model) {
        ...
        model.addAttribute("progressos",
                progressoTrilhaService.listarPorUsuario(usuario.getIdUsuario()));
